...
const colors = ['#e8574d', '#4287f5', '#54f542', '#f59e42', '#f55442'];

// Het balobject (de kubus die heen en weer stuitert)
const Ball = {
...
// Het ai-object (de twee lijnen die op en neer bewegen)
const Ai = {
    new(side) {
        return {
            width: 18,
...
const Game = {
    ...
    menu() {
        ...
        this.context.fillText('Druk spacebar om te beginnen',
            this.canvas.width / 2,
            this.canvas.height / 2 + 15
        );
    },
    ...
    update() {
        if (!this.over) {
            ...
            if (this.ball.x >= this.canvas.width - this.ball.width) this._resetTurn(this.player, this.ai);
        ...
        if (this.player.score === rounds[this.round]) {
            if (!rounds[this.round + 1]) {
                this.over = true;
                setTimeout(() => { this.endGameMenu('Je hebt gewonnen!'); }, 1000);
            } else {
            ...
            this.over = true;
            setTimeout(() => { this.endGameMenu('Je hebt verloren :('); }, 1000);
        }
    },
